...
var opCodes = map[int]func(c *Computer, direct []bool){
	...
	3: func(c *Computer, direct []bool) {
		...
		c.SetMemoryValue(c.InstructionPointer+1, c.Inputs[c.InputPointer])
		c.InputPointer += 1
...
func NewComputer(initialMemory []int) *Computer {
	...
	im := make([]int, len(initialMemory))
	copy(im, initialMemory)
...
func LoadProgram(path string) ([]int, error) {
...
func ComputerFromFile(path string) (*Computer, error) {
	...
	return NewComputer( program ), nil
}
